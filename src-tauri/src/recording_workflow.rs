...
impl RecordingWorkflow {
                ...
                match command {
                    RecordingCommand::StartRecording {
                        ...
                        performance_tracker_for_iter
                            ...
                            .await;
                        performance_tracker_for_iter
                            .track_event("recording_command", "StartRecording command received")
                            .await;

                        // Capture active app context on macOS
                        performance_tracker_for_iter
                            .track_event("app_context", "Capturing active app context")
                            .await;
                        #[cfg(target_os = "macos")]
                        ...
                        if let Some(ref ctx) = app_context {
                            info(
                                Component::Recording,
                                &format!(
                                    "Recording started in app: {} ({})",
                                    ctx.name, ctx.bundle_id
                                ),
                            );
                        }
                        ...
                        performance_tracker_for_iter
                            .track_event("whisper_logging", "Starting whisper log session")
                            .await;
                        // Set the session ID for the log interceptor
                        WhisperLogInterceptor::set_session_id(Some(session_id.clone()));
                        let whisper_log_path =
                            match whisper_logger::start_whisper_session(&session_id) {
                                Ok(path) => {
                                    debug(
                                        Component::Recording,
                                        &format!("Started whisper log session: {}", session_id),
                                    );
                                    Some(path)
                                }
                                Err(e) => {
                                    warn(
                                        Component::Recording,
                                        &format!("Failed to start whisper logging: {}", e),
                                    );
                                    None
                                }
                            };
                        ...
                        info(
                            Component::Recording,
                            "Initializing transcription context for real-time chunking...",
                        );
                        performance_tracker_for_iter
                            .track_event("transcription_init", "Creating transcription context")
                            .await;
                        ...
                        let transcription_context =
                            match TranscriptionContext::new_from_db_with_readiness(
                            ...
                            .await
                            {
                                ...
                                Err(e) => {
                                    warn(
                                        Component::Recording,
                                        &format!("Failed to create transcription context: {}", e),
                                    );
                                    info(
                                        Component::Recording,
                                        "Falling back to traditional processing queue",
                                    );
                                    ...
                                    match recorder.start_recording(&path, device_name.as_deref()) {
                                        Ok(_) => {
                                            ...
                                            progress_tracker_for_iter.update(
                                        ...
                                        Err(e) => {
                                            progress_tracker_for_iter
                                                .update(RecordingProgress::Idle);
                            ...
                            Ok(_) => {
                                ...
                                let sample_rx_option = if strategy_name == "ring_buffer" {
                                    ...
                                    info(
                                        Component::RingBuffer,
                                        &format!(
                                            "Detected {} strategy - connecting audio samples",
                                            strategy_name
                                        ),
                                    );
                                    ...
                                    let audio_channels =
                                        device_info.as_ref().map(|d| d.channels).unwrap_or(2)
                                            as usize;
                                    drop(recorder_guard);
                                    ...
                                    if std::env::var_os("DISABLE_SAMPLE_CALLBACKS").is_some() {
                                        ...
                                        // Set the callback on the recorder within a tight scope
                                        let set_result = {
                                            let recorder_guard = recorder.lock().await;
                                            recorder_guard.set_sample_callback(Some(sample_callback))
                                        };
                                ...
                                // Start recording
                                performance_tracker_for_iter
                                    .track_event("audio_start", "Starting audio recording")
                                    .await;
                                // Start the recorder without holding the lock across heavy work
                                let start_result = {
                                    let recorder_guard = recorder.lock().await;
                                    recorder_guard.start_recording(&path, device_name.as_deref())
                                };
                                match start_result {
                                    Ok(_) => {
                                        let start_time = std::time::Instant::now();
                                        ...
                                        info(
                                            Component::Recording,
                                            &format!(
                                                "Started recording with transcription strategy: {}",
                                                strategy_name
                                            ),
                                        );
                                        ...
                                        if let Some(mut sample_rx) = sample_rx_option {
                                            ...
                                            if let Some(ring_buffer) = transcription_context.get_ring_buffer() {
                                                info(
                                                    Component::RingBuffer,
                                                    "Got ring buffer reference successfully",
                                                );
                                                ...
                                                tokio::spawn(async move {
                                                    ...
                                                    while let Some(samples) = sample_rx.recv().await
                                                    {
                                                        ...
                                                        let before_count =
                                                            ring_buffer_clone.sample_count();

                                                        // Feed samples directly to ring buffer
                                                        if let Err(e) =
                                                            ring_buffer_clone.add_samples(&samples)
                                                        {
                                                    ...
                                                    if let Err(e) =
                                                        ring_buffer_clone.finalize_recording()
                                                    {
                                                ...
                                                info(
                                                    Component::RingBuffer,
                                                    "Ring buffer sample processing task started",
                                                );
                                            } else {
                                    ...
                                    Err(e) => {
                                        // Stop recording if transcription setup failed
                                        {
                                            ...
                                            let _ = recorder_guard.set_sample_callback(None);
                                        }
                    ...
                    RecordingCommand::StopRecording { response } => {
                        info(
                            Component::Recording,
                            "RecordingWorkflow: StopRecording command received",
                        );
                        performance_tracker_for_iter
                            .track_event("recording_command", "StopRecording command received")
                            .await;
                        if let Some(mut active_recording) = current_recording.take() {
                            let duration_ms =
                                active_recording.start_time.elapsed().as_millis() as i32;
                            info(
                                Component::Recording,
                            ...
                            // Get device info and stop recorder in a tight scope
                            let device_info = {
                                let recorder_guard = recorder.lock().await;
                                let device_info_snapshot = recorder_guard.get_current_device_info();
                                debug(Component::Recording, "Calling recorder.stop_recording()...");
                                performance_tracker_for_iter
                                    .track_event("audio_stop", "Stopping audio recorder")
                                    .await;
                                ...
                                // Clear sample callback to prevent accumulation across recordings
                                if let Err(e) = recorder_guard.set_sample_callback(None) {
                                    warn(
                                        Component::Recording,
                                ...
                                device_info_snapshot  // Return the device info captured before stopping
                            };
                            ...
                            match crate::audio::WavValidator::validate_wav_file(&wav_path) {
                                Ok(validation_result) => {
                                    ...
                                    if validation_result.has_errors() {
                                        ...
                                        info(
                                            Component::Recording,
                                            "WAV file validation passed - no issues detected",
                                        );
                                    }
                                }
                                Err(e) => {
                                    error(
                                        Component::Recording,
                                        &format!("Failed to validate WAV file: {}", e),
                                    );
                                }
                            }
                            ...
                            if let Some(mut transcription_context) =
                                active_recording.transcription_context.take()
                            {
                                let strategy_name = transcription_context
                                    .current_strategy_name()
                                    .unwrap_or_else(|| "unknown".to_string());
                                ...
                                let whisper_log_path_clone =
                                    active_recording.whisper_log_path.clone();
                                ...
                                tokio::spawn(async move {
                                    perf_tracker_clone
                                        .track_event(
                                            "transcription_start",
                                            "Starting async transcription processing",
                                        )
                                        .await;

                                    // Emit transcribing status for native overlay
                                    let transcribing_status =
                                        crate::processing_queue::ProcessingStatus::Transcribing {
                                            filename: filename_clone.clone(),
                                        };
                                    if let Err(e) = app_handle_clone
                                        .emit("processing-status", &transcribing_status)
                                    {
                                        error(
                                            Component::UI,
                                            &format!("Failed to emit transcribing status: {:?}", e),
                                        );
                                    } else {
                                        info(
                                            Component::UI,
                                            "Emitted transcribing status for native overlay",
                                        );
                                    }
                                    ...
                                    perf_tracker_clone
                                        .track_event(
                                            ...
                                            "Calling finish_recording on transcription context",
                                        )
                                        .await;
                                    ...
                                    match finish_timeout {
                                        Ok(Ok(transcription_result)) => {
                                            ...
                                            let speed_ratio = duration_ms as f64
                                                / transcription_result.processing_time_ms as f64;
                                            info(
                                                Component::Transcription,
                                                &format!(
                                                    "Transcription completed: {} chars in {:.2}s",
                                                    transcription_result.text.len(),
                                                    transcription_result.processing_time_ms as f64
                                                        / 1000.0
                                                ),
                                            ...
                                            perf_tracker_clone
                                                .track_event(
                                                    "post_processing",
                                                    "Starting post-processing hooks",
                                                )
                                                .await;
                                            let post_processing =
                                            ...
                                            if let Some(ref ctx) = active_recording.app_context {
                                                info(
                                                    Component::Recording,
                                                    &format!(
                                                        "Adding app context to metadata: {} ({})",
                                                        ctx.name, ctx.bundle_id
                                                    ),
                                                );
                                                ...
                                                info(
                                                    Component::Recording,
                                                    "No app context available to add to metadata",
                                                );
                                            }
                                            ...
                                            info(
                                                Component::Recording,
                                                "App context not available on non-macOS platform",
                                            );

                                            let metadata = metadata_json.to_string();
                                            debug(
                                                Component::Recording,
                                                &format!("Final metadata JSON: {}", metadata),
                                            );
                                            ...
                                            match database_clone
                                                .save_transcript_with_audio_metadata(
                                                    &filtered_transcript,
                                                    duration_ms,
                                                    Some(&metadata),
                                                    audio_metadata_json.as_deref(),
                                                    Some(
                                                        &recordings_dir_clone
                                                            .join(&filename_clone)
                                                            .to_string_lossy(),
                                                    ),
                                                ...
                                                .await
                                            {
                                                ...
                                                .with_strategy_metadata(serde_json::json!({
                                                    ...
                                                    if let Some(ref device_info) =
                                                        device_info_result
                                                    {
                                                        perf_builder = perf_builder
                                                            .with_device_info(device_info);
                                                    }

                                                    let performance_data = perf_builder.build();

                                                    if let Err(e) = post_processing
                                                    ...
                                                    if let Some(timeline_events) =
                                                        perf_tracker_clone
                                                            .get_timeline_for_database(
                                                                &session_id_clone,
                                                            )
                                                            .await
                                                    {
                                                        if let Err(e) = database_clone
                                                            .save_performance_timeline_events(
                                                                transcript.id,
                                                                &session_id_clone,
                                                                timeline_events,
                                                            )
                                                    ...
                                                    info(
                                                        ...
                                                        "Updated progress tracker to Idle state",
                                                    );
                                                    ...
                                                    if let Err(e) = app_handle_clone
                                                        .emit("transcript-created", &transcript)
                                                    {
                                                    ...
                                                    debug(
                                                        Component::UI,
                                                        "Emitting processing-complete event",
                                                    );
                                                    if let Err(e) = app_handle_clone
                                                        .emit("processing-complete", &transcript)
                                                    {
                                                    ...
                                                    debug(
                                                        Component::UI,
                                                        "Emitting recording-completed event",
                                                    );
                                                    if let Err(e) = app_handle_clone
                                                        .emit("recording-completed", &transcript.id)
                                                    {
                                                    ...
                                                    if let Some(summary) =
                                                        perf_tracker_clone.end_session().await
                                                    {
                                                    ...
                                                    if let Some(_log_path) = whisper_log_path_clone
                                                    {
                                                        ...
                                                        tokio::spawn(async move {
                                                            ...
                                                            if let Some(final_log_path) =
                                                                whisper_logger::end_whisper_session(
                                                                    &session_id,
                                                                )
                                                            {
                                        ...
                                        Ok(Err(e)) => {
                                            ...
                                            info(
                                                ...
                                                &format!(
                                                    ...
                                                    fallback_duration.as_secs_f64()
                                                ),
                                            );
                                            error(
                                                Component::Transcription,
                                                "Strategy Failed: ring_buffer",
                                            );
                                            error(
                                                Component::Transcription,
                                                &format!("Reason: {}", e),
                                        ...
                                        Err(_) => {
                                            error(
                                                Component::Transcription,
                                                "Transcription timeout after 3 minutes!",
                                            );
                    ...
                    RecordingCommand::CancelRecording { response } => {
                        if let Some(mut active_recording) = current_recording.take() {
                            // Stop recording (tight lock scope)
                            {
                                let recorder_guard = recorder.lock().await;
                                if let Err(e) = recorder_guard.stop_recording() {
                                    let _ = response.send(Err(e));
                                ...
                                // Clear sample callback on cancel
                                if let Err(e) = recorder_guard.set_sample_callback(None) {
                                    warn(
                                        Component::Recording,
                            ...
                            if let Some(_transcription_context) =
                                active_recording.transcription_context.take()
                            {
                            ...
                            let audio_path =
                                recordings_dir_for_iter.join(&active_recording.filename);
                            if let Err(e) = tokio::fs::remove_file(&audio_path).await {
                                error(
                                    Component::Recording,
                                    &format!("Failed to delete cancelled recording: {}", e),
                                );
                            }
    ...
    pub async fn start_recording(&self, device_name: Option<String>) -> Result<String, String> {
        ...
        self.command_tx
            .send(RecordingCommand::StartRecording {
                device_name,
    ...
    pub async fn stop_recording(&self) -> Result<RecordingResult, String> {
        let (response_tx, response_rx) = oneshot::channel();
        self.command_tx
            .send(RecordingCommand::StopRecording {
                response: response_tx,
            })
            .await
            .map_err(|_| "Failed to send command".to_string())?;
    ...
    pub async fn cancel_recording(&self) -> Result<(), String> {
        let (response_tx, response_rx) = oneshot::channel();
        self.command_tx
            .send(RecordingCommand::CancelRecording {
                response: response_tx,
            })
            .await
            .map_err(|_| "Failed to send command".to_string())?;
