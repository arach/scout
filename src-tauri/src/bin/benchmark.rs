...
use scout_lib::benchmarking::{BenchmarkRunner, TestDataExtractor, StrategyTester, TranscriptionStrategy, MetricsAnalyzer};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("✅ Benchmark completed! Results saved to: {}", output_dir.display());
    Ok(())
}
...
async fn extract_test_data(database: Arc<Database>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for test in &tests {
        println!("  - {} ({:?}, {} ms)", test.name, test.recording_length_category, test.duration_ms);
    }
    ...
    println!("💾 Test data configuration saved to: {}", config_path.display());
    Ok(())
}
...
async fn run_strategy_benchmark(
    ...
    println!("  Avg time to first result: {:.2}ms", report.summary.avg_time_to_first_result);
    println!("  Avg total time: {:.2}ms", report.summary.avg_total_time);
    
    println!("\n🎯 Strategy Comparisons:");
    for comparison in &analysis.strategy_comparisons {
        println!("  {} ({}x):", comparison.strategy_name, comparison.test_count);
...
async fn run_cutoff_benchmark(
    ...
    for (name, cutoff_ms) in cutoff_tests {
        ...
        println!("  ✅ {} completed: {} successful / {} total", 
...
async fn run_progressive_benchmark(
    ...
    let strategies = vec![
        ...
        TranscriptionStrategy::RingBuffer { chunk_size_ms: 5000 },
        TranscriptionStrategy::RingBuffer { chunk_size_ms: 1000 },
    ...
    for strategy in strategies {
        ...
        if !strategy_results.is_empty() {
            let avg_ttfr = strategy_results.iter()
                .map(|r| r.timing_metrics.time_to_first_result_ms as f32)
                .sum::<f32>() / strategy_results.len() as f32;
            
            println!("  ✅ Average TTFR: {:.2}ms ({} tests)", avg_ttfr, strategy_results.len());
        }
    }
