use chrono;
use scout_lib::benchmarking::{RecordingLength, TestDataExtractor};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for category in [
        RecordingLength::UltraShort,
        RecordingLength::Short,
        RecordingLength::Medium,
        RecordingLength::Long,
    ] {
        let category_recordings: Vec<_> = all_tests
            .iter()
            .filter(|r| {
                std::mem::discriminant(&r.recording_length_category)
        ...
        println!(
            "📊 Selected {} recordings from {:?} category",
    ...
    println!(
        "✅ Total selected for corpus: {} recordings\n",
    ...
    for (index, recording) in selected_recordings.iter().enumerate() {
        ...
        let audio_copied = if recording.audio_file.exists() {
            ...
            println!(
                "   ⚠️ Source audio file not found: {}",
                recording.audio_file.display()
            );
            false
        };
        ...
        match benchmark_database
            .save_transcript(
                &recording
                    .expected_transcript
                    .as_ref()
                    .unwrap_or(&"No transcript available".to_string()),
                recording.duration_ms as i32,
                Some(&format!(
                    "category:{:?},original_id:{}",
                    recording.recording_length_category, recording.name
                )),
    ...
    fs::write(
        &gitignore_path,
        "# Benchmark corpus - do not commit\n*\n!.gitignore\n",
    )?;
