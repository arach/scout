...
struct QualityStatistics {
    chunk_size_1000ms: ChunkSizeStats,
    chunk_size_2000ms: ChunkSizeStats,
    chunk_size_3000ms: ChunkSizeStats,
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("🎯 COMPREHENSIVE CHUNKING ANALYSIS");
    println!("===================================\\n");
    
    // Initialize database and get ALL test recordings
    ...
    println!("📋 Analyzing ALL {} recordings with comprehensive chunking", all_tests.len());
    ...
    for (idx, recording) in all_tests.iter().enumerate() {
        println!("🎵 [{}/{}] COMPREHENSIVE ANALYSIS: {} ({}ms, {:?})", 
                idx + 1, all_tests.len(), recording.name, recording.duration_ms, recording.recording_length_category);
    ...
    let report = ComprehensiveChunkingReport {
        ...
        recordings_analyzed,
        summary: ComprehensiveSummary {
            total_recordings: all_tests.len(),
    ...
    let output_file = "./comprehensive_chunking_analysis.json";
...
fn generate_quality_statistics(recordings: &[ComprehensiveChunkingAnalysis]) -> QualityStatistics {
    let mut stats_1000 = Vec::new();
    let mut stats_2000 = Vec::new();
    let mut stats_3000 = Vec::new();
    ...
    for recording in recordings {
        for chunk_analysis in &recording.chunk_analyses {
            ...
            match chunk_analysis.chunk_size_ms {
                1000 => stats_1000.push((similarity, wer)),
                2000 => stats_2000.push((similarity, wer)),
                3000 => stats_3000.push((similarity, wer)),
                _ => {}
            }
        }
    }
    ...
    QualityStatistics {
        chunk_size_1000ms: calculate_chunk_stats(&stats_1000),
        chunk_size_2000ms: calculate_chunk_stats(&stats_2000),
        chunk_size_3000ms: calculate_chunk_stats(&stats_3000),
    }
}
...
fn generate_comprehensive_recommendations(recordings: &[ComprehensiveChunkingAnalysis]) -> Vec<String> {
    ...
    recommendations.push(format!(
        "1000ms chunks: {:.1}% professional quality rate ({:.1}% WER avg) - AVOID",
        stats.chunk_size_1000ms.success_rate_professional * 100.0,
        stats.chunk_size_1000ms.avg_word_error_rate * 100.0
    ));
    
    recommendations.push(format!(
        "2000ms chunks: {:.1}% professional quality rate ({:.1}% WER avg) - USE WITH CAUTION",
        stats.chunk_size_2000ms.success_rate_professional * 100.0,
        stats.chunk_size_2000ms.avg_word_error_rate * 100.0
    ));
    
    recommendations.push(format!(
        "3000ms chunks: {:.1}% professional quality rate ({:.1}% WER avg) - RECOMMENDED",
        stats.chunk_size_3000ms.success_rate_professional * 100.0,
        stats.chunk_size_3000ms.avg_word_error_rate * 100.0
    ));
    
    // Determine best chunk size
    if stats.chunk_size_3000ms.success_rate_professional >= 0.8 {
        recommendations.push("RECOMMENDATION: Use 3000ms chunks for production (best quality)".to_string());
    } else if stats.chunk_size_2000ms.success_rate_professional >= 0.6 {
        recommendations.push("RECOMMENDATION: Use 2000ms chunks for production (acceptable quality)".to_string());
    } else {
        recommendations.push("WARNING: Consider longer chunks or different approach - current options have low success rates".to_string());
    }
...
fn print_comprehensive_analysis(report: &ComprehensiveChunkingReport) {
    ...
    println!("🔸 1000ms chunks:");
    println!("   Average similarity: {:.3}", report.summary.quality_statistics.chunk_size_1000ms.avg_similarity);
    println!("   Average WER: {:.1}%", report.summary.quality_statistics.chunk_size_1000ms.avg_word_error_rate * 100.0);
    println!("   Professional quality (>90%): {:.1}%", report.summary.quality_statistics.chunk_size_1000ms.success_rate_professional * 100.0);
    
    println!("🔸 2000ms chunks:");
    println!("   Average similarity: {:.3}", report.summary.quality_statistics.chunk_size_2000ms.avg_similarity);
    println!("   Average WER: {:.1}%", report.summary.quality_statistics.chunk_size_2000ms.avg_word_error_rate * 100.0);
    println!("   Professional quality (>90%): {:.1}%", report.summary.quality_statistics.chunk_size_2000ms.success_rate_professional * 100.0);
    
    println!("🔸 3000ms chunks:");
    println!("   Average similarity: {:.3}", report.summary.quality_statistics.chunk_size_3000ms.avg_similarity);
    println!("   Average WER: {:.1}%", report.summary.quality_statistics.chunk_size_3000ms.avg_word_error_rate * 100.0);
    println!("   Professional quality (>90%): {:.1}%", report.summary.quality_statistics.chunk_size_3000ms.success_rate_professional * 100.0);
