...
impl ProfanityFilter {
    ...
    pub fn filter_transcript(
        &self,
        text: &str,
        recording_duration_ms: Option<i32>,
    ) -> FilterResult {
        ...
        for word in &words {
            ...
            if self.hallucination_words.contains(&clean_word) {
                ...
                if !likely_hallucination {
                    let (is_hallucination, mut logs) = self.is_likely_hallucination(
                        &original_text,
                        &clean_word,
    ...
    fn is_likely_hallucination(
        ...
        if text_lower.starts_with(profanity_word)
            || text_lower.starts_with(&format!("oh {}", profanity_word))
        {
            let log_msg = format!(
                "🚫 Profanity '{}' at start of transcript - likely hallucination",
                profanity_word
            );
            warn(Component::Processing, &log_msg);
            analysis_logs.push(log_msg);
        ...
        if word_count == 1 && self.hallucination_words.contains(profanity_word) {
            let log_msg = format!(
                "🚫 Single profanity word '{}' - likely hallucination",
                profanity_word
            );
            warn(Component::Processing, &log_msg);
            analysis_logs.push(log_msg);
        ...
        if text_lower.contains("oh, fuck")
            || text_lower.contains("oh fuck")
            || text_lower.contains("oh, shit")
            || text_lower.contains("oh shit")
        {
            let log_msg = format!(
                "🚫 Common hallucination pattern detected with '{}' - likely hallucination",
                profanity_word
            );
            warn(Component::Processing, &log_msg);
            analysis_logs.push(log_msg);
        ...
        let log_msg = format!(
            "✅ Profanity '{}' appears intentional based on context",
            profanity_word
        );
        info(Component::Processing, &log_msg);
        analysis_logs.push(log_msg);
    ...
    fn remove_profanity(&self, text: &str) -> String {
        ...
        let cleaned = filtered_result.replace("  ", " ").trim().to_string();
...
mod tests {
    ...
    fn test_hallucination_detection() {
        ...
        let result =
            filter.filter_transcript("I can't believe I forgot my fucking keys again", Some(5000));
        assert!(!result.likely_hallucination);
        assert!(result.profanity_detected);
        assert_eq!(
            result.filtered_text,
            "I can't believe I forgot my fucking keys again"
        );
        assert!(!result.analysis_logs.is_empty());
    }
