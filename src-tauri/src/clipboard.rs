...
pub fn copy_to_clipboard(text: &str) -> Result<(), String> {
    info(Component::UI, &format!("🔄 Starting copy operation for text: {} characters", text.len()));
    
    if text.is_empty() {
        error(Component::UI, "❌ Copy operation failed: Cannot copy empty text to clipboard");
    ...
    let mut clipboard_guard = CLIPBOARD.lock().map_err(|e| {
        error(Component::UI, &format!("❌ Failed to acquire clipboard lock: {}", e));
    ...
    if let Some(ref mut clipboard) = clipboard_guard.as_mut() {
        ...
        clipboard.set_text(text).map_err(|e| {
            error(Component::UI, &format!("❌ Failed to write to clipboard: {}", e));
            format!("Failed to copy to clipboard: {}", e)
        })?;
        info(Component::UI, &format!("✅ Text written to clipboard: {} characters", text.len()));
        ...
        match clipboard.get_text() {
            ...
            Err(e) => {
                error(Component::UI, &format!("❌ Failed to verify clipboard content: {}", e));
                Err(format!("Failed to verify clipboard content: {}", e))
            }
        }
    } else {
        error(Component::UI, "❌ Clipboard not available - initialization failed");
...
pub fn simulate_paste() -> Result<(), String> {
    ...
    if !permission_check.status.success() {
        ...
        error(Component::UI, &format!("❌ Accessibility permissions check failed: {}", error_msg));
    ...
    let frontmost_app = String::from_utf8_lossy(&permission_check.stdout).trim().to_string();
    info(Component::UI, &format!("🖥️ Frontmost application detected: '{}'", frontmost_app));
    ...
    if frontmost_app.contains("scout") || frontmost_app.contains("Scout") {
        info(Component::UI, "⏭️ Skipping auto-paste because Scout is the frontmost application");
        return Ok(());
    }
    
    info(Component::UI, &format!("🎯 Attempting to paste to application: '{}'", frontmost_app));
    ...
    let output = Command::new("osascript")
        ...
        .output()
        .map_err(|e| {
            error(Component::UI, &format!("❌ Failed to execute AppleScript: {}", e));
            format!("Failed to execute AppleScript: {}", e)
        })?;
    
    if !output.status.success() {
        let error_msg = String::from_utf8_lossy(&output.stderr);
        error(Component::UI, &format!("❌ AppleScript paste command failed: {}", error_msg));
        return Err(format!("AppleScript failed: {}", error_msg));
    }
    
    info(Component::UI, &format!("✅ Successfully simulated paste command to '{}'", frontmost_app));
    Ok(())
}
