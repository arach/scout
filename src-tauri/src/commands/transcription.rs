...
pub async fn test_external_service_with_audio(state: State<'_, AppState>) -> Result<serde_json::Value, String> {
    ...
    // Try to send audio through ZeroMQ
    match send_audio_to_zeromq(&audio_data, sample_rate, &external_config).await {
        Ok(transcription) => {
            Ok(serde_json::json!({
                "success": true,
                "message": "Successfully sent audio and received transcription",
                "transcription": transcription,
                "details": {
                    "audio_duration_ms": (duration_secs * 1000.0) as u32,
                    "sample_rate": sample_rate,
                    "samples": num_samples,
                    "model": external_config.model,
                    "workers": external_config.workers
                }
            }))
        }
        Err(e) => {
            Ok(serde_json::json!({
                "success": false,
                "message": format!("Failed to test with audio: {}", e)
            }))
        }
    }
}

/// Helper function to send audio to ZeroMQ and get transcription
async fn send_audio_to_zeromq(
    audio_data: &[f32],
    sample_rate: u32,
    config: &crate::settings::ExternalServiceConfig,
) -> Result<String, String> {
    use std::time::Duration;
    
    // Run ZMQ operations in a blocking task since zmq crate is not async
    tokio::task::spawn_blocking(move || {
        ...
        push_socket.connect(&format!("tcp://127.0.0.1:{}", config.zmq_push_port))
            .map_err(|e| format!("Failed to connect to push socket: {}", e))?;
        
        // Create pull socket for receiving transcription
        let pull_socket = ctx.socket(zmq::PULL)
            .map_err(|e| format!("Failed to create pull socket: {}", e))?;
        pull_socket.connect(&format!("tcp://127.0.0.1:{}", config.zmq_pull_port))
            .map_err(|e| format!("Failed to connect to pull socket: {}", e))?;
        
        // Set receive timeout
        pull_socket.set_rcvtimeo(100)
            .map_err(|e| format!("Failed to set receive timeout: {}", e))?;
        
        // Prepare audio message with metadata
        let audio_message = serde_json::json!({
            "audio": audio_data,
            "sample_rate": sample_rate,
            "session_id": format!("test_{}", chrono::Utc::now().timestamp_millis()),
            "timestamp": chrono::Utc::now().to_rfc3339(),
        });
        
        let message_bytes = serde_json::to_vec(&audio_message)
            .map_err(|e| format!("Failed to serialize audio message: {}", e))?;
        
        // Send audio data
        push_socket.send(&message_bytes, 0)
        ...
        loop {
            if start_time.elapsed() > timeout_duration {
                return Err("Timeout waiting for transcription".to_string());
            }
            ...
            match pull_socket.recv_bytes(0) {
                Ok(bytes) => {
                    ...
                    if let Some(text) = result.get("text").and_then(|t| t.as_str()) {
                        ...
                        return Err(format!("Transcription error: {}", error));
                    } else {
                        return Err("Invalid transcription response format".to_string());
                    }
                }
                ...
                Err(e) => {
                    return Err(format!("Failed to receive transcription: {}", e));
                }
            }
        }
    })
