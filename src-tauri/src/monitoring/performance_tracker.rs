...
impl PerformanceTimeline {
    pub fn new(session_id: String) -> Self {
        ...
        timeline.add_event(
            "session_started",
            &format!("Performance tracking started for session {}", session_id),
        );
        timeline
    }
    ...
    pub fn add_event(&mut self, event_type: &str, details: &str) {
        ...
        info(
            Component::Recording,
            &format!("[{}] +{}ms: {}", event_type, duration_ms, details),
        );
    ...
    pub fn get_summary(&self) -> String {
        ...
        for event in &self.events {
            ...
            summary.push_str(&format!(
                "[{:>4}ms] +{:>3}ms {} - {}\n",
                elapsed, delta, event.event_type, event.details
            ));
        }
...
impl PerformanceTracker {
    ...
    pub async fn get_timeline_for_database(
        &self,
        session_id: &str,
    ) -> Option<Vec<(String, String, String, Option<i64>)>> {
        let timelines = self.timelines.lock().await;
        if let Some(timeline) = timelines.iter().find(|t| t.session_id == session_id) {
            let events: Vec<(String, String, String, Option<i64>)> = timeline
                .events
                .iter()
                .map(|event| {
                    (
