...
impl ModelStateManager {
    ...
    pub async fn update_coreml_state(&self, model_id: &str, state: CoreMLState) {
        ...
        if let Some(model_state) = states.get_mut(model_id) {
            ...
            states.insert(model_id.to_string(), ModelState {
...
pub async fn warm_coreml_models(
    model_state_manager: Arc<ModelStateManager>,
    models_dir: PathBuf,
) {
    info(Component::Models, "Starting background Core ML model warming");
    ...
    for (model_id, state) in states {
        if matches!(state.coreml_state, CoreMLState::Downloaded) {
            info(Component::Models, &format!("Warming Core ML for model: {}", model_id));
            
            // Update state to warming
            model_state_manager.update_coreml_state(&model_id, CoreMLState::Warming).await;
            ...
            tokio::spawn(async move {
                match warm_single_model(&model_id_clone, &models_dir_clone).await {
                    Ok(_) => {
                        info(Component::Models, &format!("Successfully warmed Core ML for: {}", model_id_clone));
                        manager_clone.update_coreml_state(&model_id_clone, CoreMLState::Ready).await;
                    }
                    Err(e) => {
                        warn(Component::Models, &format!("Failed to warm Core ML for {}: {}", model_id_clone, e));
                        manager_clone.update_coreml_state(&model_id_clone, CoreMLState::Failed(e)).await;
                    }
                }
            });
        }
