...
impl AudioNotificationService {
    ...
    pub fn initialize(&mut self, app_handle: AppHandle) {
        ...
        info(
            Component::Recording,
            "Audio notification service initialized",
        );
    }
        ...
        if let Some(ref app_handle) = self.app_handle {
            ...
            match severity {
                NotificationSeverity::Critical => {
                    error(
                        Component::Recording,
                        &format!("CRITICAL AUDIO ISSUE: {}", event_name),
                    );
                }
                NotificationSeverity::Error => {
                    error(
                        Component::Recording,
                        &format!("AUDIO ERROR: {}", event_name),
                    );
                }
                NotificationSeverity::Warning => {
                    warn(
                        Component::Recording,
                        &format!("AUDIO WARNING: {}", event_name),
                    );
                }
            ...
            if let Err(e) = app_handle.emit("audio-notification", &event) {
                error(
                    Component::Recording,
                    &format!("Failed to emit audio notification: {}", e),
                );
            }
        } else {
            warn(
                Component::Recording,
                "Audio notification service not initialized - cannot send notification",
            );
        }
    }
    ...
    pub fn notify_airpods_issues(&self, device_name: String, sample_rate: u32) {
        ...
        let mut recommendations =
            vec!["For best audio quality, consider using wired headphones".to_string()];
        let severity;
    ...
    fn get_event_severity(&self, event: &AudioNotificationEvent) -> NotificationSeverity {
        match event {
            ...
            AudioNotificationEvent::DeviceCapabilitiesChanged { .. } => {
                NotificationSeverity::Warning
            }
            AudioNotificationEvent::DeviceDisconnected {
                recording_interrupted,
                ..
            } => {
    ...
    fn get_event_name(&self, event: &AudioNotificationEvent) -> String {
        match event {
            AudioNotificationEvent::ProblematicDeviceDetected {
                device_name,
                device_type,
                ..
            } => {
            ...
            AudioNotificationEvent::SampleRateMismatch {
                device_name,
                reported_rate,
                detected_rate,
                ..
                ...
                if let Some(detected) = detected_rate {
                    format!(
                        "Sample rate mismatch on {}: reported {} Hz, detected {} Hz",
                        device_name, reported_rate, detected
                    )
                } else {
                    format!("Sample rate issue on {}: {} Hz", device_name, reported_rate)
                }
            }
            AudioNotificationEvent::AudioQualityIssue {
                device_name,
                issue_type,
                ..
            } => {
