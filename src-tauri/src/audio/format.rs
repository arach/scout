...
use super::notifications::{notify_sample_rate_mismatch, notify_bluetooth_detected};
...
impl WhisperAudioConverter {
    ...
    pub fn convert_for_whisper(
        ...
        let whisper_samples = if native_format.sample_rate != 16000 {
            ...
            let resampled = Self::resample_with_validation(&mono_samples, resample_ratio, native_format)?;
    ...
    fn validate_input_format(samples: &[f32], native_format: &NativeAudioFormat) -> Result<(), String> {
        ...
        if invalid_count > 0 {
            return Err(format!("Invalid audio samples detected: {} non-finite values out of {}", 
                invalid_count, samples.len()));
        }
        ...
        if native_format.sample_rate < 8000 || native_format.sample_rate > 192000 {
            return Err(format!("Unusual sample rate detected: {} Hz", native_format.sample_rate));
        }
    ...
    fn detect_device_specific_issues(samples: &[f32], native_format: &NativeAudioFormat) -> Result<(), String> {
        let device_name_lower = native_format.device_name.to_lowercase();
        
        // AirPods specific checks
        if device_name_lower.contains("airpod") {
            info(Component::Transcription, "AirPods device detected - applying enhanced validation");
            
            // Check for call mode (low sample rates)
            if native_format.sample_rate <= 24000 {
                warn(Component::Transcription, &format!("AirPods in call mode: {} Hz - audio may have limited quality", 
                    native_format.sample_rate));
                
                // Analyze signal for potential sample rate mismatch
                if let Some(detected_rate) = Self::estimate_actual_sample_rate(samples, native_format.sample_rate) {
                    if (detected_rate as i32 - native_format.sample_rate as i32).abs() > 8000 {
                        ...
                        error(Component::Transcription, "This will cause chipmunk effect or slow audio!");
                        ...
                        return Err(format!("Critical sample rate mismatch: reported {} Hz, detected {} Hz", 
                            native_format.sample_rate, detected_rate));
                    }
                }
            }
            ...
            if Self::is_audio_silent(samples) {
                warn(Component::Transcription, "AirPods audio appears silent - possible connection issue");
            }
        }
        
        // Bluetooth device checks
        if device_name_lower.contains("bluetooth") || device_name_lower.contains("wireless") {
            info(Component::Transcription, "Bluetooth device detected - checking for common issues");
            
            // Check for intermittent dropouts (common in Bluetooth)
            let has_dropouts = Self::detect_audio_dropouts(samples);
            if has_dropouts {
                warn(Component::Transcription, "Audio dropouts detected - possible Bluetooth interference");
            }
    ...
    fn resample_with_validation(samples: &[f32], ratio: f32, native_format: &NativeAudioFormat) -> Result<Vec<f32>, String> {
        if ratio == 1.0 {
            return Ok(samples.to_vec());
        }
        
        info(Component::Transcription, &format!("Resampling with ratio {:.3} for device: {}", ratio, native_format.device_name));
        
        // Check for problematic ratios that might indicate sample rate issues
        if ratio < 0.1 || ratio > 10.0 {
            warn(Component::Transcription, &format!("Extreme resampling ratio: {:.3} - possible sample rate mismatch", ratio));
        }
