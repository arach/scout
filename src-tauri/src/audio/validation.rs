...
impl AudioFormatValidator {
    ...
    pub fn process_callback(&mut self, samples: &[f32], callback_info: &CallbackInfo) -> ValidationResult {
    ...
    fn perform_validation(&mut self, callback_info: &CallbackInfo) -> ValidationResult {
        ...
        if let Some(detected_rate) = self.estimate_sample_rate() {
            ...
            if rate_diff > 1000 {
                issues.push(ValidationInconsistency {
                    ...
                    details: format!("Expected: {} Hz, Detected: {} Hz (diff: {} Hz)", 
                                   self.expected_sample_rate, detected_rate, rate_diff),
                    severity: if rate_diff > 8000 { InconsistencySeverity::Critical } else { InconsistencySeverity::High },
                });
            }
        }
        ...
        if issues.is_empty() {
            ...
            let max_severity = issues.iter().map(|i| &i.severity).max().unwrap_or(&InconsistencySeverity::Low).clone();
    ...
    fn perform_quick_checks(&mut self, samples: &[f32], _callback_info: &CallbackInfo) {
        ...
        if samples.len() > 100 && samples.iter().all(|&s| s.abs() < 1e-10) {
            ...
            let recent_silent = self.signal_levels.iter().rev().take(10).all(|&level| level < 1e-8);
    ...
    fn estimate_sample_rate(&self) -> Option<u32> {
        ...
        let avg_crossings: f32 = self.zero_crossings.iter().sum::<u32>() as f32 / self.zero_crossings.len() as f32;
        ...
        let estimated_freq = (avg_crossings / 2.0) * (self.expected_sample_rate as f32 / samples_per_callback);
    ...
    fn validate_signal_consistency(&self) -> Option<ValidationInconsistency> {
        ...
        let older_levels: Vec<f32> = self.signal_levels.iter().rev().skip(10).take(10).cloned().collect();
            ...
            if older_avg > 1e-6 && recent_avg > 1e-6 {
                ...
                if ratio > 5.0 || ratio < 0.2 {
                    return Some(ValidationInconsistency {
                        ...
                        detected_at: Instant::now(),
                        details: format!("Signal level changed by {}x (from {:.6} to {:.6})", 
    ...
    fn validate_channel_configuration(&self, callback_info: &CallbackInfo) -> Option<ValidationInconsistency> {
        ...
        if callback_info.samples_received != expected_samples_per_callback {
            return Some(ValidationInconsistency {
                ...
                detected_at: Instant::now(),
                details: format!("Expected {} samples ({} frames × {} channels), got {}", 
                               expected_samples_per_callback, callback_info.frames, 
                               self.expected_channels, callback_info.samples_received),
    ...
    fn validate_callback_timing(&self, callback_info: &CallbackInfo) -> Option<ValidationInconsistency> {
        // Calculate expected time between callbacks
        let expected_interval_ms = (callback_info.frames as f32 / self.expected_sample_rate as f32) * 1000.0;
        ...
        if (actual_interval_ms - expected_interval_ms).abs() > tolerance && actual_interval_ms > 1.0 {
            return Some(ValidationInconsistency {
                ...
                detected_at: Instant::now(),
                details: format!("Expected ~{:.1}ms between callbacks, got {:.1}ms", 
    ...
    pub fn generate_pattern_analysis(&self) -> Option<AudioPatternAnalysis> {
        ...
        let avg_signal_level = self.signal_levels.iter().sum::<f32>() / self.signal_levels.len() as f32;
        ...
        let sample_rate_confidence = if self.inconsistencies.iter()
    ...
    pub fn get_validation_stats(&self) -> ValidationStats {
        let critical_count = self.inconsistencies.iter().filter(|i| i.severity == InconsistencySeverity::Critical).count();
        let high_count = self.inconsistencies.iter().filter(|i| i.severity == InconsistencySeverity::High).count();
        let medium_count = self.inconsistencies.iter().filter(|i| i.severity == InconsistencySeverity::Medium).count();
        let low_count = self.inconsistencies.iter().filter(|i| i.severity == InconsistencySeverity::Low).count();
