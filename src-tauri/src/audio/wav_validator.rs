...
impl WavValidator {
    ...
    pub fn validate_wav_file(path: &Path) -> Result<WavValidationResult, String> {
        ...
        info(
            Component::Recording,
            &format!("WAV Validation for: {:?}", path),
        );
        info(
            Component::Recording,
            &format!(
        ...
        let silent_samples = first_samples
            .iter()
        ...
        if first_samples.len() >= 100 {
            ...
            let estimated_freq = (zero_crossings as f32 / 2.0)
                * (spec.sample_rate as f32 / first_samples.len() as f32);
    ...
    pub fn log_issues(&self) {
        for issue in &self.issues {
            match issue.severity {
                Severity::Info => info(
                    Component::Recording,
                    &format!("WAV Validation Info: {}", issue.message),
                ),
                Severity::Warning => warn(
                    Component::Recording,
                    &format!("WAV Validation Warning: {}", issue.message),
                ),
                Severity::Error => error(
                    Component::Recording,
                    &format!("WAV Validation Error: {}", issue.message),
                ),
            }
        }
    }
