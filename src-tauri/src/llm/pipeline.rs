use super::{LLMEngine, GenerationOptions, PromptTemplate};
use anyhow::Result;
...
impl LLMPipeline {
    ...
    pub async fn process_transcript(
        ...
        for template in templates {
            info(Component::Processing, &format!("Processing transcript with prompt: {}", template.name));
...
pub fn chunk_transcript(transcript: &str, max_tokens: usize) -> Vec<String> {
    ...
    for sentence in sentences {
        ...
        if current_chunk.split_whitespace().count() + word_count > max_words && !current_chunk.is_empty() {
