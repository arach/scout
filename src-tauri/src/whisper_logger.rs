...
static WHISPER_SESSION_LOGGER: Lazy<Arc<Mutex<WhisperSessionLogger>>> = Lazy::new(|| {
    Arc::new(Mutex::new(WhisperSessionLogger::new()))
});
...
pub fn init_whisper_logger(log_dir: &Path) -> Result<(), String> {
    ...
    info(Component::Transcription, &format!("Whisper session logs will be written to: {:?}", whisper_log_dir));
    
    Ok(())
}

pub fn start_whisper_session(session_id: &str) -> Result<PathBuf, String> {
    let mut logger = WHISPER_SESSION_LOGGER.lock()
    ...
    logger.active_sessions.insert(session_id.to_string(), session);
    
    debug(Component::Transcription, &format!("Started whisper session '{}', log file: {:?}", session_id, log_path));
...
pub fn end_whisper_session(session_id: &str) -> Option<PathBuf> {
    ...
    if let Some(mut session) = logger.active_sessions.remove(session_id) {
        ...
        let _ = writeln!(session.file, "Session '{}' ended at {}", 
                       session_id, end_time.format("%Y-%m-%d %H:%M:%S"));
        ...
        debug(Component::Transcription, &format!("Ended whisper session '{}' with {} entries", session_id, session.entry_count));
    ...
    if let Ok(mut logger) = WHISPER_SESSION_LOGGER.lock() {
        if let Some(session) = logger.active_sessions.get_mut(session_id) {
            ...
            if let Some(ref meta) = entry.metadata {
                let _ = writeln!(session.file, "  metadata: {}", serde_json::to_string(meta).unwrap_or_default());
            }
