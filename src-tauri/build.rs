    ...
    if env::var("CARGO_CFG_TARGET_OS").unwrap_or_default() == "macos" {
        ...
        let output = std::process::Command::new("swiftc")
            .args([
                ...
                swift_header_path.to_str().unwrap(),
                "-module-name",
                "overlay",
                "-import-objc-header",
        ...
        let output = std::process::Command::new("swiftc")
            .args([
                ...
                "-emit-objc-header",
                "-emit-objc-header-path",
                out_dir.join("app_context-Swift.h").to_str().unwrap(),
                "-module-name",
                "app_context",
                "-o",
        ...
        let output = std::process::Command::new("swiftc")
            .args([
                ...
                "NativeOverlay",
                &format!(
                    "{}/src/macos/native_overlay/Logger.swift",
                ...
                &format!(
                    "{}/src/macos/native_overlay/OverlayViewController.swift",
                    env!("CARGO_MANIFEST_DIR")
                ),
                &format!(
                    "{}/src/macos/native_overlay/OverlayBridge.swift",
                    env!("CARGO_MANIFEST_DIR")
                ),
            ])
            .output()
        ...
        std::env::set_current_dir(env!("CARGO_MANIFEST_DIR"))
            .expect("Failed to change directory back");
