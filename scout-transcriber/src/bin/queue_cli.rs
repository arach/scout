...
/// CLI tool for interacting with Scout Transcriber queues
#[derive(Parser)]
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Push { audio, sample_rate, channels } => {
            let input_queue = SledQueue::<AudioChunk>::new(&PathBuf::from("/tmp/scout-transcriber/input"))?;
        ...
        Commands::Pop { queue } => {
            match queue.as_str() {
                "input" => {
                    let queue = SledQueue::<AudioChunk>::new(&PathBuf::from("/tmp/scout-transcriber/input"))?;
                ...
                "output" => {
                    let queue = SledQueue::<Result<Transcript, TranscriptionError>>::new(
                        &PathBuf::from("/tmp/scout-transcriber/output")
                    )?;
