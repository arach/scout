//! Transcriber - A standalone transcription service
//! 
...
/// Errors that can occur in the transcriber system
#[derive(Error, Debug)]
...
/// Default queue paths
pub const DEFAULT_INPUT_QUEUE_PATH: &str = "/tmp/transcriber/input";
pub const DEFAULT_OUTPUT_QUEUE_PATH: &str = "/tmp/transcriber/output";
...
mod tests {
    ...
    async fn test_transcriber_client_creation() {
        ...
        let result = TranscriberClient::with_paths(
            "/tmp/test-input-transcriber",
            "/tmp/test-output-transcriber"
        );
        assert!(result.is_ok());
    }
}
